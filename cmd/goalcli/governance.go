...
func runVersion(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	return emitReport(stdout, "version", "passed", []string{"kafkax release " + projectReleaseVersion, "goalcli governance runtime " + governanceRuntimeVersion, "goalcli governance CLI available"}, nil)
}
...
func appendAgentIndexGaps(path string, gaps *[]string) {
	...
	text := string(content)
	for _, needle := range []string{"schema_version:", "module: kafkax", "control_plane:", "files:"} {
		if !strings.Contains(text, needle) {
			*gaps = append(*gaps, path+" missing "+needle)
		}
	}
