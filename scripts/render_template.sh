...
Renders kafkax into a concrete base library by copying the repository,
moving pkg/kafkax to pkg/<package>, and replacing template identifiers.
USAGE
}
...
if [[ "$package_name" != "kafkax" ]]; then
  mkdir -p "$out_dir/pkg"
  mv "$out_dir/pkg/kafkax" "$out_dir/pkg/$package_name"
fi
...
replace_in_text_files 'kafkax_' "${package_name}_"
