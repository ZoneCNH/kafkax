...
if [[ "$package_name" != "kafkax" && -e "$repo_dir/pkg/kafkax" ]]; then
  echo "ERROR: stale pkg/kafkax directory still exists" >&2
  exit 1
fi
...
scan_template_placeholders
scan_fixed "github.com/ZoneCNH/kafkax" "module path"
scan_fixed "github.com/ZoneCNH/kafkax" "module path"

if [[ "$module_name" != "kafkax" ]]; then
  scan_fixed "kafkax" "module name"
fi
...
if [[ "$package_name" != "kafkax" ]]; then
  scan_fixed "pkg/kafkax" "package directory reference"
  scan_fixed "kafkax_" "metrics prefix"
  scan_fixed "Kafkax" "title-case package name"
  scan_fixed "KAFKAX" "upper-case package name"
  scan_regex '\bkafkax\b' "package name"
fi
