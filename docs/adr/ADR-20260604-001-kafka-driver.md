...
`kafkax` 要作为 Kafka L2 adapter/factory 标准推进，但公共 API 不能绑定任意第三方 Kafka client。当前仓库已经有 contract-first 的 `pkg/kafkax` skeleton、JSON schema 和文档 gate；尚未在本工区出现 `internal/driver`、fake driver、默认生产 driver 或 broker-backed fixtures。

## 决策

- 在内部实现层建立 `internal/driver` driver-neutral 边界，用标准 Go 类型表达 producer、consumer 和 admin 行为。
...
- `testkit` 必须先提供 fake driver/fixture，用于无 broker、无第三方 Kafka 依赖的 contract tests。broker-backed adapter 只能在 fake gate 通过后进入。
...
- REQ-004 的完成条件包括 `internal/driver` 接口、fake driver/testkit fixtures、公共 API contract tests 和 broker gate 状态；单独新增文档或 skeleton 不足以声明完成。
- REQ-011 的 release/adoption 证据必须等待 runtime、release manifest、score gate 和 downstream adoption artifacts，不得由 ADR 替代。
...
- Fake-driver gate：`internal/driver` contract tests 与 `testkit` fake fixtures，无 broker、无第三方 Kafka client。
