# kafkax 完整 Goal 可执行方案 v2.9.3 Complete

> 版本：v2.9.3 Complete
> 目标仓库：`https://github.com/ZoneCNH/kafkax`  
...
2. **命名与自符合风险**：旧名 `kafkax` / `foundationx` 缺少明确 guard；且下游 adoption 前应先完成 `kafkax` 自身 standard-source profile attestation。
...
LIMIT-006  不必所有下游同时迁移；先 kafkax self，再 kernel，再 configx。
```
...
git worktree add ../.worktrees/kafkax/<issue-id>   -b <branch-name> main
```
...
title: kafkax v2.9.3 Constitution Runtime & Conformance Automation
mode: full
owner: lead
...
success_criteria:
  ...
  - kafkax self-conformance attestation is generated before downstream adoption.
...
| P1-021 | Naming Consistency and Legacy Name Guard    | docs/governance   | C2         | .agent/policies/naming.yaml; docs/standard/naming.md; internal/goalcli/naming/\*\*                                                                                                                                                               | goalcli naming; docs-check                                                | 默认名称统一 kafkax/kernel；旧名 kafkax/foundationx 仅允许迁移/ADR/兼容上下文 |
...
| P2-015 | kafkax Self-Conformance Attestation | conformance         | C3         | release/manifest/conformance-attestation.json; docs/reports/self-conformance.md                                                       | goalcli attest-conformance --profile standard-source                                                          | 下游 adoption 前先证明 kafkax 自身符合 standard-source profile                               |
...
file_classes:
  XLIB_OWNED:
    description: kafkax may generate and update
  USER_OWNED:
...
```text
1. kafkax self-conformance -> standard-source
2. kernel -> l0-kernel
3. configx -> l1-shared
```
...
```text
standard repo: kafkax
L0 downstream: kernel
old template name: kafkax only allowed in migration docs context
...
| RISK-013 | kafkax 自身未证明符合就要求下游符合 |    P1 | self-conformance attestation             |
...
3. kafkax self-conformance attestation exists.
...
3. kafkax self-conformance attestation 生成。
...
```text
You are executing GOAL-20260602-001 for github.com/ZoneCNH/kafkax.
