...
func TestBuildDockerEvidenceReturnsDefaultsWhenEnvUnset(t *testing.T) {
	...
	if got.ToolchainImage != "kafkax-toolchain:local" {
		t.Fatalf("docker.toolchain_image = %q, want kafkax-toolchain:local", got.ToolchainImage)
	}
	...
	if got.RuntimeImage != "kafkax-goalcli-runtime:local" {
		t.Fatalf("docker.runtime_image = %q, want kafkax-goalcli-runtime:local", got.RuntimeImage)
	}
