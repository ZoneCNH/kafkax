...
func TestNewErrorFormatsKindOpAndMessage(t *testing.T) {
	err := NewError(ErrorKindValidation, "kafkax.Test", "bad input", false)
	if err == nil {
	...
	if got := err.Error(); !strings.Contains(got, "validation: kafkax.Test: bad input") {
...
func TestWrapErrorPreservesCauseAndKind(t *testing.T) {
	...
	err := WrapError(ErrorKindTimeout, "kafkax.Test", "", true, cause)
...
func TestErrorHandlesNilReceiverAndCauseOnlyMessage(t *testing.T) {
	...
	err := &Error{Kind: ErrorKindInternal, Op: "kafkax.Test", Cause: cause}
	if got := err.Error(); got != "internal: kafkax.Test: root cause" {
...
func TestErrorKindFallbacksForPlainErrors(t *testing.T) {
	...
	if IsKind(err, ErrorKindInternal) {
		t.Fatal("plain errors should not match kafkax error kinds")
	}
...
func TestContextErrorClassifiesDeadlineAsRetryableTimeout(t *testing.T) {
	err := contextError("kafkax.Test", context.DeadlineExceeded)
	if !IsKind(err, ErrorKindTimeout) {
		t.Fatalf("expected timeout kind, got %v", err)
	}
	if !err.Retryable {
